class Telemetric {
    static async init(project_id_param, version, track_on_localhost_param = false) {
        ...
        if (window.location.hostname === "localhost" && !this.track_on_localhost) {
            console.log("Telemetric initialized successfully, but will not send any data on Localhost.");
            return;
        }
        try {
    ...
    static async event(name) {
        if (!this.safetyCheck(`Event '${name}'`))
            return;
        if (window.location.hostname === "localhost" && !this.track_on_localhost) {
    ...
    static safetyCheck(source) {
        ...
        if (!this.user_id) {
            console.error(`${source} reporting failed. Missing user ID. Make sure to call init() before tracking events or revenue. Also make sure to await init()`);
            isSafe = false;
        }
        return isSafe;
    }
...
Telemetric.project_id = null;
Telemetric.user_id = null;
Telemetric.version = null;
Telemetric.track_on_localhost = false;
Telemetric.initial = false;
