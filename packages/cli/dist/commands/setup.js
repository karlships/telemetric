...
export async function setup(framework) {
    try {
        // Validate framework
        if (!["next", "react"].includes(framework)) {
            console.error('Unsupported framework. Please use "next" or "react"');
            process.exit(1);
        }
        ...
        // Copy provider
        const providerContent = await fs.readFile(path.join(templateDir, "provider.tsx"), "utf-8");
        await fs.writeFile(path.join(providersDir, "TelemetricProvider.tsx"), providerContent);
        // Copy telemetric core
        const telemetricContent = await fs.readFile(path.join(__dirname, "../core/telemetric.ts"), "utf-8");
        await fs.writeFile(path.join(providersDir, "telemetric.ts"), telemetricContent);
